...
import { QuestionnaireResponse, SectionState } from "@/types";
import { ProgressBar } from "@/components/ProgressBar";
import { SectionNavigation } from "@/components/SectionNavigation";
import { QuestionRenderer } from "@/components/QuestionRenderer";
import { SaveIndicator } from "@/components/SaveIndicator";
import { CompletionScreen } from "@/components/CompletionScreen";
import { getResponse, saveResponse, createInitialResponse, calculateProgress, clearResponse } from "@/lib/localStorage";
...
export default function QuestionnairePage() {
  ...
  }, [currentSectionId, response]);

  const loadResponse = (): QuestionnaireResponse | null => {
    try {
      let data = getResponse();
      
      if (!data) {
        data = createInitialResponse();
      }
      
      setResponse(data);
      if (data.progress.currentSection) {
        const sectionId = `section${data.progress.currentSection}`;
  ...
  const persistResponse = useCallback(
    (sectionId: string, answers: any, completed?: boolean) => {
      if (!response) return;

      setIsSaving(true);

      try {
        let currentSection = response.sections[sectionId];
        ...
        if (completed !== undefined) {
          updatedSection.completed = completed;
          updatedSection.completedAt = completed ? new Date().toISOString() : null;
        }
        ...
        updatedResponse.progress = calculateProgress(updatedResponse.sections);

        // Check if all sections are complete
        if (updatedResponse.progress.completedSections === updatedResponse.progress.totalSections) {
          updatedResponse.completedAt = new Date().toISOString();
        }
  ...
  const debouncedSave = useCallback(
    (sectionId: string, answers: any) => {
      ...
      saveTimeoutRef.current = setTimeout(() => {
        persistResponse(sectionId, answers);
      }, 500);
    },
    [persistResponse]
  );
  ...
  const handleSectionComplete = async () => {
    ...
    persistResponse(currentSectionId, section.answers, true);
  };
  ...
  const handleCompleteQuestionnaire = async () => {
    ...
    const updatedResponse = loadResponse();
    if (!updatedResponse) {
  ...
  const handleStartOver = async () => {
    ...
    try {
      clearResponse();
      const freshResponse = createInitialResponse();
  ...
  if (showCompletion && response) {
    return <CompletionScreen response={response} />;
  }
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              if (currentSectionState) {
                                persistResponse(currentSectionId, currentSectionState.answers, false);
                              }
                            }
                          }}
